...
public class GvrPointerInputModuleImpl {
  ...
  public void Process() {
    ...
    if (IsPointerActiveAndAvailable()) {
            ...
            triggerDown = (Pointer.TriggerDown) ? (Pointer.TriggerDown) : (Input.anyKeyDown && Input.inputString.Equals(System.Environment.NewLine)) ? (Input.anyKeyDown && Input.inputString.Equals(System.Environment.NewLine)) : Input.GetButtonDown("Fire1") ? true : Input.GetMouseButtonDown(0); // True if the trigger is held down. 
