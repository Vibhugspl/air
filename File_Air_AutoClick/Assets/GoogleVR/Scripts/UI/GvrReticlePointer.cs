...
public class GvrReticlePointer : GvrBasePointer {
  ...
  public float RETICLE_DISTANCE_MIN = 0.2f;
  // Maximum distance of the reticle (in meters).
  public float RETICLE_DISTANCE_MAX = 100.0f;
  ...
  public float ReticleOuterDiameter { get; private set; }
	...
	/// //////////////////////////////////
	/// </summary>
	/// <value>The max pointer distance.</value>
  ...
  public override void OnPointerHover(RaycastResult raycastResultResult, bool isInteractive) {
    SetPointerTarget(raycastResultResult.worldPosition, isInteractive);
		if (gazedAt)
			 reticle.fillAmount = ((Time.time - gazedTime) / 2);
		       // if(gazedAt = null )
			        //{
			           //gazedTime = -1f;
			           // gazedAt = null;
		        ...
		        if (gazedAt && gazedAt.GetComponent< EventTrigger>() && gazedTime > 0 && (Time.time - gazedTime) > 2)
			        {
			                Debug.Log("ONGaze");
			            gazedTime = -1f;
			            EventTrigger eventtrigger = gazedAt.GetComponent< EventTrigger>();

			            eventtrigger.OnPointerDown( pointer);
			            eventtrigger.OnPointerClick( pointer);
			        }
  ...
  public override void GetPointerRadius(out float enterRadius, out float exitRadius) {
    ...
    enterRadius = 0.5f * Mathf.Tan(min_inner_angle_radians);
  ...
  public void UpdateDiameters() {
    ...
    float inner_half_angle_radians = Mathf.Deg2Rad * ReticleInnerAngle * 0.3f;
    float outer_half_angle_radians = Mathf.Deg2Rad * ReticleOuterAngle * 0.3f;
